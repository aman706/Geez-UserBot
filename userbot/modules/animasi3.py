...
from Ironx import bot, CMD_HELP, ALIVE_NAME
...
CHASE_STR = [
    ...
    "Once upon a time, in a galaxy far far away... Someone would care about that, But not anymore.",
...
SLAP_TEMPLATES_EN = [
    ...
    "tie {victim} to the stake and whip them {where} with {item}."
...
WHERE_EN = ["on the cheek", "on the head", "on the buttocks", "on the body"]
...
WHERE_ID = ["on the cheek", "on the head", "on the buttocks", "on the body"]
...
@register(outgoing=True, pattern=r"^\.butt(?: |$)(.*)")
async def butts(e):
     await e.edit("`Sin, Get Beautiful Ass Pictures...`")
     wait sleep(3)
     await e.edit("`Sending Beautiful Ass Pictures...`")
     nsfw = requests.get(
         'http://api.obutts.ru/noise/1').json()[0]["Butt Picture"]
     urllib.request.urlretrieve(
